-- 1) New columns on consolidation_records (additive, non-destructive)
ALTER TABLE public.consolidation_records
  ADD COLUMN IF NOT EXISTS stage TEXT NOT NULL DEFAULT 'visitante',
  ADD COLUMN IF NOT EXISTS visit_date DATE,
  ADD COLUMN IF NOT EXISTS consolidation_start_date DATE,
  ADD COLUMN IF NOT EXISTS consolidation_end_date DATE,
  ADD COLUMN IF NOT EXISTS baptism_date DATE,
  ADD COLUMN IF NOT EXISTS contact_evaluation TEXT;

CREATE INDEX IF NOT EXISTS idx_consolidation_records_stage
  ON public.consolidation_records (church_id, stage);

-- 2) Update reset_module_data: add 'consolidacao' fix + 'all' (geral) option
...
BEGIN
  ...
  CASE p_module
    ...
    WHEN 'all' THEN
      -- Reset GERAL: limpa todos os módulos exceto a igreja, profiles vinculados, roles e configurações de sistema
      DELETE FROM public.cell_report_attendance WHERE report_id IN (SELECT id FROM public.cell_reports WHERE church_id = p_church_id);
      DELETE FROM public.cell_reports WHERE church_id = p_church_id;
      DELETE FROM public.cell_leadership_development WHERE church_id = p_church_id;
      DELETE FROM public.cell_pastoral_care WHERE church_id = p_church_id;
